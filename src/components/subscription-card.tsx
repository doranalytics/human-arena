...
export function SubscriptionCard({ compact = false }: { compact?: boolean }) {
  ...
  return <div className={compact ? "rounded-lg border border-line bg-bg-2/70 px-3.5 py-3" : "rounded-xl border border-line bg-bg-2/70 p-4"}>
    <div className="flex items-center gap-2 text-[15px] font-medium">{paid && <Check size={16} className="text-ok" />}{paid ? "How to AI membership confirmed" : "Keep learning with How to AI"}</div>
    <p className="mt-1 text-[13px] leading-relaxed text-ink-2">{paid ? (viaCircle ? "Your Circle membership includes weekly winner eligibility. You’re already included." : "Your membership is confirmed for weekly winner eligibility.") : "Play the challenges for free. Paid subscribers can qualify for the weekly winner feature in front of a million people on Ruben’s LinkedIn."}</p>
    <div className="mt-3 flex flex-wrap items-center gap-3 text-[13px]">
      {(!paid || viaSubstack) && <a href={paid ? SUBSCRIPTION_SETTINGS_URL : SUBSCRIBE_URL} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-1.5 rounded-lg bg-ink px-3 py-1.5 font-medium text-bg">{paid ? "Manage on Substack" : "Subscribe on Substack"}<ExternalLink size={13} /></a>}

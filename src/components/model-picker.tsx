...
import { MODELS, EFFORTS, DEFAULT_EFFORT, type ModelChoice, type Effort } from "@/lib/models";
            ...
            {modelOpen && (
              <div className={cn("mobile-popover fade-up absolute z-30 w-72 rounded-2xl border border-line bg-bg p-1.5 shadow-lg shadow-black/10", menusDown ? "top-11 left-0" : "bottom-11 right-0")}>
                ...
                {(Object.keys(MODELS) as ModelChoice[]).map((k) => (
                  <button key={k} type="button" onClick={() => { updateSettings({ model: k, effort: DEFAULT_EFFORT[k] }); setModelOpen(false); }} className="flex w-full items-start gap-3 rounded-xl px-3 py-2.5 text-left hover:bg-bg-2">

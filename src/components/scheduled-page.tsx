            ...
            <div key={sc.id} className="rounded-2xl border border-line bg-bg p-4">
              <div className="flex flex-wrap items-start gap-3">
                ...
                <div className="flex w-full shrink-0 items-center justify-end gap-1 md:w-auto">
                  <button onClick={() => { const c = runSchedule(sc.id); if (c) { setPage(null); openChat(c.id); } }} className="flex h-10 items-center gap-1.5 md:h-8 rounded-lg bg-ink px-3 text-[12.5px] font-medium text-bg hover:bg-black"><Play size={13} /> Run now</button>

  ...
  return (
    <aside className="flex h-full w-[272px] shrink-0 flex-col border-r border-line bg-side">
      ...
      <div className="px-2.5 pt-1">
        <NavItem icon={<Plus size={16} />} label="New" onClick={() => newChat(activeProjectId)} active={!activeChatId && !activeProjectId && !attempt} />
      ...
      <div className="mt-4 flex-1 overflow-y-auto px-2.5 pb-2">
        ...
        {projects.map((p) => (
          <button key={p.id} onClick={() => openProject(p.id)} className={cn("flex h-8 w-full items-center gap-2.5 rounded-lg px-2 text-[13.5px] hover:bg-bg-3", activeProjectId === p.id && !activeChatId && "bg-bg-3")}>
        ...
        {list.map((c) => (
          <div key={c.id} className={cn("group flex h-8 items-center rounded-lg pr-1 hover:bg-bg-3", activeChatId === c.id && "bg-bg-3")}>
            <button onClick={() => openChat(c.id)} className="flex min-w-0 flex-1 items-center gap-2.5 px-2 text-left text-[13.5px]">
